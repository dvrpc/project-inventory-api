...
def create_attachment(attachment_in: AttachmentCreateRequest, db: Session = Depends(get_db), admin=Depends(require_admin)):
    return create(db, attachment_in)

@router.put("/{attachment_id}", response_model=AttachmentResponse)
def update_attachment(attachment_id: int, attachment_in: AttachmentUpdateRequest, db: Session = Depends(get_db), admin=Depends(require_admin)):
    attachment = get(db, attachment_id)
    if not attachment:
        raise HTTPException(status_code=404, detail="Attachment not found")
...
def delete_attachment(attachment_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
    attachment = get(db, attachment_id)
