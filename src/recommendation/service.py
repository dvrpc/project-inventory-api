...
from .schema import RecommendationCreateRequest, RecommendationUpdateRequest
from .models import Recommendation

def get(db : Session, recommendation_id: int):
    return db.query(Recommendation).filter(Recommendation.recommendation_id == recommendation_id).one_or_none()
...
def update(db: Session, recommendation: Recommendation, recommendation_in: RecommendationUpdateRequest):
