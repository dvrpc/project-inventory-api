...
from src.external_product.models import ExternalProduct


def get(db: Session, external_product_id: int):
    return (
        db.query(ExternalProduct)
        .filter(ExternalProduct.product_id == external_product_id)
        .one_or_none()
    )
...
def update(
    ...
    external_product_in: ExternalProductUpdateRequest,
):
