...
def create_project_keyword(project_keyword_in: ProjectKeywordCreateRequest, db: Session = Depends(get_db), admin=Depends(require_admin)):
    return create(db, project_keyword_in)

@router.delete("/{project_id}/{keyword_id}")
def delete_project_keyword(project_id: int, keyword_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)):
